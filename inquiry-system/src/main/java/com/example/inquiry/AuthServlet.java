...
public class AuthServlet extends HttpServlet {
	...
	private static final String ADMIN_USER = "admin";//ログインに使うユーザー名を指定
	private static final String ADMIN_PASS = "123";//パスワードを指定

	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		String username = req.getParameter("username");
		String password = req.getParameter("password");//それぞれ受け取ったものを二つの変数に代入
		if (ADMIN_USER.equals(username) && ADMIN_PASS.equals(password)) { //上で指定したユーザー名・パスワードと代入したものが一致していたら以下の処理
			...
			resp.sendRedirect(req.getContextPath() + "/inquiry?action=history");//リダイレクトの命令内でパスを指定
		} else { //一致していなければ以下の処理
